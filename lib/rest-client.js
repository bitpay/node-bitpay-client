...
BitPay.prototype._sendRequest = function() {
  ...
  options.headers = {
    'x-pubkey': KeyUtils.getPubKey(this.privkey),
    'x-signature': KeyUtils.sign(contract, this.privkey),
...
BitPay.prototype._processResponse = function(entity, path) {
  ...
  var path    = (path.indexOf(entity.id) !== -1) ? path + '/' : path + '/' + entity.id + '/';
...
BitPay.prototype._resourceArgs = function() {
  var entity   = this;
  var args     = Array.prototype.slice.call(arguments);
  var path     = entity.path + args[0];
  var data     = (typeof args[1] === 'function') ? {} : args[1];
  var lastArg  = args[args.length - 1];
  var callback = (typeof lastArg === 'function') ? lastArg : new Function();
