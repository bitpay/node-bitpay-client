...
var RESTClient = function(privateKey, options) {
  ...
  if (!privateKey) {
    console.warn('No private key specified. Only public methods allowed.');
  }
...
RESTClient.prototype._sendRequest = function() {
  ...
  var useSSL   = (this.apiPort === 443 || this.options.config.forceSSL);
  ...
  if (self.options.signRequests && this.facade !== 'public') {
    options.headers['x-pubkey']    = bitauth.getPublicKeyFromPrivateKey(this.privkey);
    options.headers['x-signature'] = bitauth.sign(contract, this.privkey);
  }
